...
def from_yaml(config_path: str) -> Dict[str, Any]:
    """Run STT benchmarks from YAML config.
...
def _run(config: dict) -> Dict[str, Any]:
    """Run STT benchmarks based on config dict."""
...
def _run_benchmarks(config: dict) -> List[Dict[str, Any]]:
    ...
    for run_cfg in config.get("benchmark", []):
        name = run_cfg.get("name", "stt_benchmark")
        engine = run_cfg.get("engine", "stt")

        if engine != "stt":
            print(f"Skipping {name}: engine '{engine}' not supported (only 'stt' supported)")
            continue
        ...
        model = (
            serve_cfg.pop("model", None)
            or serve_cfg.pop("model_path", None)
...
def _generate_result_name(config_name: str, index: int, bench_cfg: dict) -> str:
    """Generate a unique result name for STT benchmark."""
    cfg_str = str(sorted(bench_cfg.items()))
    cfg_hash = hashlib.md5(cfg_str.encode()).hexdigest()[:6]

    parts = [config_name]

    if "num_requests" in bench_cfg:
        parts.append(f"n{bench_cfg['num_requests']}")
    if "max_concurrency" in bench_cfg:
        parts.append(f"c{bench_cfg['max_concurrency']}")
    if "request_rate" in bench_cfg:
        parts.append(f"rps{bench_cfg['request_rate']}")
